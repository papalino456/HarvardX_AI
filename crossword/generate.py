...
class CrosswordCreator():
    ...
    def assignment_complete(self, assignment):
        ...
        if self.crossword.variables == set(assignment.keys()):
            return True
        if not (self.crossword.variables == set(assignment.keys())):
            return False

    def consistentChecker(self, x, y, assignment):
        overlap = self.crossword.overlaps[x, y]
        if overlap == None:
            return True
        ...
        if wordx[overlap[0]] == wordy[overlap[1]]:
            return True
        if not (wordx[overlap[0]] == wordy[overlap[1]]):
            return False
    ...
    def consistent(self, assignment):
        ...
        for x in assignment:
            for y in assignment:
                if x != y:
                    if not self.consistentChecker(x, y, assignment):
                        return False
        return True
